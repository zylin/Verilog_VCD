...
def _parse_vcd(file, only_sigs=0, use_stdout=0, siglist=[], opt_timescale=''):
    ...
    with open(file, 'r') as fh:
        while True:
            ...
            if line[0] in ('b', 'B', 'r', 'R'):
                ...
                mult, timescale = _calc_mult(statement, opt_timescale)
                ...
                if (full_name in usigs) or all_sigs:
                    if code not in data:
                        data[code] = {}
                        data[code]['nets'] = dict()
                    # Avoid O(n^2) performance by using a hash instead of a list to ensure that we
                    # don't add duplicate signals
                    data[code]['nets'][path + '.' + name] = {
                        'type': vcd_type,
                        'name': name,
                        'size': size,
                        'hier': path,
    ...
    for code in data:
        # Make it compatible
        data[code]['nets'] = list(data[code]['nets'].values())
...
def _calc_mult(statement, opt_timescale=''):
    ...
    return (((mult * scale) / new_scale), timescale)
...
class _Signal(object):
    ...
    def __init__(self, reader, signal_name):
        self._reader = reader
        self._nets, self._tv = self._reader._signals[signal_name]

    def value(self, tvalue):
        '''Get string representation, which includes xxxx for undefined-

        tvalue == -1 means startup value of signal
        '''
        if tvalue > self._reader.endtime:
            raise ValueError("Unkonwn signal at " + str(tvalue))
        ...
        for i in range(0, len(self._tv) - 1):
            if self._tv[i][0] <= tvalue < self._tv[i + 1][0]:
                return self._tv[i][1]

        # The signal after last recorded change
        return self._tv[-1][1]

        '''Get string representation, which includes xxxx for undefined'''
        return self._value(tvalue)[1]

    def int(self, tvalue):
        '''Convert value to an integer'''
        value = self.value(tvalue)
        signal_type = self._nets['type']
        if signal_type == 'wire':
            return int(value, 2)
...
class _Hierarchy(object):
    ...
    def __init__(self, reader, hierarchy):
        self._reader = reader
        self._hierarchy = hierarchy

    def __getattr__(self, name):
        signal_name = (
            name if self._hierarchy == '' else self._hierarchy + '.' + name)
        if signal_name in self._reader._signals:
            return _Signal(self._reader, signal_name)
        else:
            # Drill down
            return _Hierarchy(self._reader, signal_name)
...
class Reader(object):
    ...
    def parse(self, file, only_sigs=False, siglist=[], opt_timescale=''):
        '''Parses vcd file, throws exception on error'''
        self._data, self._timescale, self._endtime = _parse_vcd(
            file, only_sigs, 0, siglist, opt_timescale)
        #
        self._signals = dict([(signal['hier'] + '.' + signal['name'], (signal, None if only_sigs else self._data[k]['tv']))
                              for k in self._data.keys()
                              for signal in self._data[k]['nets']])
        self._tops = set([str(name).split('.')[0]
                          for name in self._signals.keys()])

    def __getattr__(self, name):
        '''Allows us to access the signal as a properties of the object'''
        if name in self._tops:
            return _Hierarchy(self, "").__getattr__(name)
        else:
...
_bigvcd = os.path.join(os.path.dirname(__file__), 'big.vcd')


class ReaderTest(unittest.TestCase):

    def test_wiki_vcd(self):
        '''Test basic parsing matches that of gtkwave'''
        # Data taken from https://en.wikipedia.org/wiki/Value_change_dump
        reader = Reader()
        reader.parse(os.path.join(os.path.dirname(__file__), 'wikipedia.vcd'))
        with self.assertRaises(ValueError):
            reader.logic.data.value(-2)
        with self.assertRaises(ValueError):
            self.assertEquals(0, reader.logic.data.int(reader.endtime + 1))
        with self.assertRaises(KeyError):
            self.assertEquals(0, reader.signal('blah'))
        self.assertEquals(2303, reader.endtime)
        self.assertEquals('xxxxxxxx', reader.logic.data.value(-1))
        self.assertEquals('10000001', reader.logic.data.value(0))
        self.assertEquals('10000001', reader.signal('logic.data').value(0))
        self.assertEquals(0x81, reader.logic.data.int(0))
        self.assertEquals(0x81, reader.logic.data.int(1500))
        self.assertEquals(0, reader.logic.data.int(reader.endtime))
        self.assertEquals(0, reader.logic.en.int(-1))
        self.assertEquals(1, reader.logic.en.int(0))
        self.assertEquals(1, reader.logic.en.int(reader.endtime))
        self.assertEquals(1, reader.logic.tx_en.int(2210), "Before edge")
        self.assertEquals(0, reader.logic.tx_en.int(2211), "On edge")
        self.assertEquals(0, reader.logic.tx_en.int(2212), "After")
        self.assertEquals(sorted(['logic.empty',
                                  'logic.en',
                                  'logic.tx_en',
                                  'logic.data',
                                  'logic.rx_en',
                                  'logic.data_valid',
                                  'logic.underrun']),
                          sorted(reader.signals))

    @unittest.skipIf(not os.path.exists(_bigvcd),
                     'Copy big.vcd alongside ' + __file__ + ', to test parsing big .vcd files')
    def test_parsing_big_vcd(self):
        '''Test parsing big .vcd file'''
        reader = Reader()
        reader.parse(_bigvcd, only_sigs=True)
        self.assertTrue(len(reader.signals) > 100000)

        # With locked down seed, read a bunch of random signals at random times to see that it doesn't
        # crap out.
        r = random.Random(1234)
        reader2 = Reader()
        reader2.parse(_bigvcd)
        self.assertTrue(len(reader2.signals) > 100000)
        signals = list(reader.signals)
        for _ in range(0, 100):
            signal_name = signals[r.randint(0, len(signals) - 1)]
            signal = reader2.signal(signal_name)
            tvalue = r.randint(-1, reader2.endtime)
            value = signal.value(tvalue)
            print(signal_name + "@" + str(tvalue) +
                  reader2.timescale + " = " + value)
